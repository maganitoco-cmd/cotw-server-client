...
Tests all 8 modules (A-H) via the /api/ proxy.
...
def api(endpoint, expect_result=1, silent=False, params_override=None):
...
# =================== MODULE H: Basic Checks ===================
section("H0. BASIC ENDPOINTS — Sanity checks")
...
if data and data.get("result") == 1:
    ok("Compra de item OK")
else:
    log("Compra de item (puede fallar si no hay recursos)")
...
if data and data.get("result") == 1:
    ...
    log("VIP levelUp (puede fallar si ya en máximo)")
...
# =================== MODULE A: Construct Timer ===================
section("A. CONSTRUCT TIMER SYSTEM")

# Start a construct — try Node=3 (may be free)
data = api(f"BUILDINGS?action=construct&deviceId={DEVICE_ID}&idUser={USER_ID}&idBuilding=1001&Node=3")
construct_id = None
if data and data.get("result") == 1:
    ...
    if "Building exist" in msg or "exist" in msg:
        ok(f"Nodo ya ocupado (esperado para test): {msg}")
        log("  Se omite verificación de timer — nodo ocupado")
    else:
        log(f"Construct (puede estar ocupado): {msg}")

# Check remainingTime
if construct_id:
    data = api(f"CONSTRUCT?action=remainingTime&deviceId={DEVICE_ID}&idUser={USER_ID}&idConstruct={construct_id}")
    if data and data.get("result") == 1:
        ok(f"remainingTime OK — User presente: {'User' in data or 'user' in data}")
    else:
        log(f"remainingTime check (puede estar completado)")
...
# Check remainingTime on all constructs (works regardless)
...
# =================== MODULE B: March Tracking ===================
section("B. MARCH TRACKING SYSTEM")

# Get quadrant (shows marches) — use kingdom from login
data = api(f"WORLDMAP?action=getQuadrant&deviceId={DEVICE_ID}&idUser={USER_ID}&x=315&y=963")
marches = []
if data and data.get("Map"):
    marches = data.get("Marches", [])
    ok(f"getQuadrant OK — {len(marches)} marchas")
else:
    ...
    if data and data.get("Map"):
        ok("getQuadrant con idKingdom=1 OK")
        marches = data.get("Marches", [])
    else:
        log("getQuadrant con idKingdom")

# Send a march (gather) — try on a closer tile
...
if data and data.get("result") == 1:
    ...
    if march_id:
        ...
        log("Marcha creada pero sin ID visible")
else:
    msg = data.get('Message', '?') if data else '?'
    log(f"Gather march: {msg} (coord puede no tener recurso)")
...
# =================== MODULE C: Quest System ===================
section("C. QUEST SYSTEM")

data = api(f"QUEST?action=feed&deviceId={DEVICE_ID}&idUser={USER_ID}")
if data and data.get("result") == 1:
    ok("Quest feed OK — Quests presente")
else:
    log("Quest feed")

# Try collectQ (collect all quest rewards)
data = api(f"QUEST?action=collectQ&deviceId={DEVICE_ID}&idUser={USER_ID}")
if data and data.get("result") == 1:
    ok("collectQ OK")
else:
    log(f"collectQ (sin rewards pendientes): {data.get('Message','?') if data else '?'}")

# Try rewards for specific quest
data = api(f"QUEST?action=rewards&deviceId={DEVICE_ID}&idUser={USER_ID}&idQuest=0")
if data and data.get("result") == 1:
    ok("Quest rewards OK")
else:
    log(f"Quest rewards (idQuest=0 puede no existir)")
...
if data and data.get("result") == 1:
    ...
    log(f"collectD (sin daily rewards): {data.get('Message','?') if data else '?'}")
...
# =================== MODULE D: Hero Management ===================
section("D. HERO MANAGEMENT")
...
data = api(f"ALLIANCE?action=get&deviceId={DEVICE_ID}&idUser={USER_ID}&idAlliance=")
...
if data and data.get("result") == 1:
    ...
    log(f"Alliance get (sin alianza)")
...
# If alliance exists, test more features
if alliance_exists:
...
if data and data.get("result") == 1:
    ...
    log(f"KingdomChat addMessage")


# =================== MODULE H: Other Endpoints ===================
section("H1. WARLORDS")
...
print(f"  Total: {passed + failed} tests | ✅ {passed} passed | ❌ {failed} failed | ⏱️ {total_time:.1f}s")
